...
class NodalBaseFnc:
    ...
    def __init__(self,foot):
        try:
            ...
            try:
                self.nodes = np.linspace(*foot["linspace"])
            except KeyError:
...
class CPPolynom(NodalBaseFnc):
    ...
    def evalD(self, x):
        out_local = np.zeros(np.shape(self.nodes))
        out_local_d = np.zeros(np.shape(self.nodes))
        if (x<=self.nodes[0]):
            out_local[0] = 1
        elif (x>=self.nodes[-1]):
            out_local[-1] = 1
            ...
            idx = np.argmax(x<self.nodes)
            out_local[idx] = (x-self.nodes[idx-1])/(self.nodes[idx]-self.nodes[idx-1])
            out_local_d[idx] = 1.0/(self.nodes[idx]-self.nodes[idx-1])
            out_local[idx-1] = 1.0-out_local[idx]
            out_local_d[idx-1] = -1.0/(self.nodes[idx]-self.nodes[idx-1])
        out_global = np.power(x, range(self.degree+1))
        out_global_d = np.zeros(self.degree+1)
        out_global_d[1:] = out_global[:-1]
        out_global_d *= np.arange(self.degree+1)
        out = np.outer(out_local_d,out_global)+np.outer(out_local,out_global_d)
        return out
...
class Fourier:
    ...
    def evalD(self,x):
        ...
        out[0] = 0
        for i in range(self.degree):
            out[1+i] = (i+1)*np.cos((i+1)*x_in)
            out[1+self.degree+i] = -(i+1)*np.sin((i+1)*x_in)
        return out
...
class RBF(NodalBaseFnc):
    """Radial basis function with gaussian interpolation
    
    """
    def __init__(self, foot):
        NodalBaseFnc.__init__(self, foot)
        self.widths = np.array(foot["widths"]) / (2 * np.sqrt(-np.log(0.5)))
    
    def gauss(self, x, u, sigma):
        return np.exp(-(x - u)**2 / (2 * sigma**2))
        
    def gaussD(self, x, u, sigma):
        return np.exp(-(x - u)**2 / (2 * sigma**2))*((u-x)/sigma**2)
    ...
    def __call__(self, x):
        ...
        if self.nrPhi == 1:
            ...
            for i in range(self.nrPhi):
                out[i] = self.gauss(x, self.nodes[i], self.widths[i])
            out = out / np.sum(out)
    ...
    def evalD(self, x):
        ...
        if self.nrPhi == 1:
            ...
            for i in range(self.nrPhi):
                out[i] = self.gaussD(x, self.nodes[i], self.widths[i])
#            out = out / np.sum(out)
...
class GLTgauss(NodalBaseFnc):
    ...
    def evalD(self, x):
        out = np.zeros(np.shape(self.nodes))
        outD = np.zeros(np.shape(self.nodes))
        if self.nrPhi==1:
            out = np.array([1])
            outD = np.array([0])
        else:
            if (x<=self.nodes[0]):
                ...
                out[0] = self.gauss(x,self.nodes[0], self.distance[0]*self.scale)
                outD[0] = self.gaussD(x,self.nodes[0], self.distance[0]*self.scale)
                
            for i in range(1,self.nrPhi-1):
                u = self.nodes[i]
                if u < x:
                    out[i] = self.gauss(x,u, self.distance[i]*self.scale)
                    outD[i] = self.gaussD(x,u, self.distance[i]*self.scale)
                else:
                    out[i] = self.gauss(x,u, self.distance[i-1]*self.scale)
                    outD[i] = self.gaussD(x,u, self.distance[i-1]*self.scale)
            ...
            if (x>=self.nodes[-1]):
                ...
                out[-1] = self.gauss(x,self.nodes[-1], self.distance[-1]*self.scale)
                outD[-1] = self.gaussD(x,self.nodes[-1], self.distance[-1]*self.scale)
            
        v = out.sum()
        vD = outD.sum()
        ret = np.zeros(np.shape(self.nodes))
        for i in range(self.nrPhi):
            ret[i] = (outD[i]*v-out[i]*vD)/(v**2)
...
class Legendre:
    ...
    def evalD(self,x):
        ...
        out[1] = 1
        for i in range(2,self.nrPhi):
            out[i] = ((2*i - 1)*x_in*out[i-1] - i*out[i-2])/(i-1)
        return out
...
class SparseUIGLTlinear():
    ...
    def __call__(self, x):
        ...
        phi = index-ratio
        if phi>0:
            return np.array([phi,1-phi]).flatten(),[index-1,index]
        else:
            return np.array([1]),[index]
    
    def idx2pos(self, idx):
        return - self.offset + self.res*idx
        
if __name__=='__main__':
    a = SparseUIGLTlinear({"res":1})
    xmin = 0
    xmax = 1
    nrNodes = 6
    x_res = 101
    nodes = np.linspace(xmin,xmax,nrNodes)
    test = GLTgauss({"nodes":nodes})
    alpha = [0,1,2,2,0,3]
    x = np.linspace(xmin,xmax,x_res)
    outPhi = np.zeros([x_res,nrNodes])
    outPhiD = np.zeros([x_res,nrNodes])
    for i in xrange(x_res):
        outPhi[i,:] = test(x[i])
        outPhiD[i,:] = test.evalD(x[i])
